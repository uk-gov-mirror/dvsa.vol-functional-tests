...
Feature: Remove last Transport Manager (TM) pop up
  ...
  Scenario: Pop up should not be displayed when a licence contains multiple TM's
  ...
  Scenario: Pop up should not be displayed when a self-serve user removes a TM
    Given the licence has been granted
    When a self-serve user removes the last TM
    Then the remove last TM popup should not be displaying new TM remove text

  Scenario: Pop up should not be displayed when a TM is removed from an application
    When the last transport manager has been removed by an internal user
    Then the remove last TM popup should not be displaying new TM remove text

  Scenario: Pop up should not be displayed when a licence contains multiple TM's
    Given i have an application
    When i remove a transport manager
    Then a pop up should not be displayed
