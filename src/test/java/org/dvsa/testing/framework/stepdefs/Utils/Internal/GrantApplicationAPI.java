...
import static org.dvsa.testing.framework.stepdefs.Utils.External.CreateInterimGoodsLicenceAPI.getOrganisationId;
...
public class GrantApplicationAPI {
    ...
    int version = 1;
    private static List outstandingFeesIds;
    private static int feeId;
    ...
    public void createOverview() {
        ...
        String overviewResource = String.format("application/%s/overview/", getApplicationNumber());
        ...
        do {
            ...
            OverviewBuilder overview = new OverviewBuilder().withId(getApplicationNumber()).withVersion(version).withLeadTcArea(transportArea).withOverrideOppositionDate(overrideOption)
    ...
    public void getOutstandingFees() {
        String getOutstandingFeesResource = String.format("application/%s/outstanding-fees/", getApplicationNumber());
    ...
    public void payOutstandingFees(){
        ...
        FeesBuilder feesBuilder = new FeesBuilder().withFeeIds(outstandingFeesIds).withOrganisationId(getOrganisationId()).withApplicationId(getApplicationNumber())
    ...
    public void grant() {
        String grantApplicationResource = String.format("application/%s/grant/", getApplicationNumber());
        GrantApplicationBuilder grantApplication = new GrantApplicationBuilder().withId(getApplicationNumber()).withDuePeriod("9").withCaseworkerNotes("This notes are from the API");
    ...
    public void payGrantFees(){
        ...
        String payOutstandingFeesResource = "transaction/pay-outstanding-fees/";
        FeesBuilder feesBuilder = new FeesBuilder().withFeeIds(Collections.singletonList(feeId)).withOrganisationId(getOrganisationId()).withApplicationId(getApplicationNumber())
                .withPaymentMethod(paymentMethod).withReceived(feesAmount).withReceiptDate(dateTimeCreate()).withPayer(payer).withSlipNo(slipNo);
        apiResponse = RestUtils.post(feesBuilder,baseURL.concat(payOutstandingFeesResource), getHeaders());
        assertThat(apiResponse.statusCode(HttpStatus.SC_CREATED));
    }
